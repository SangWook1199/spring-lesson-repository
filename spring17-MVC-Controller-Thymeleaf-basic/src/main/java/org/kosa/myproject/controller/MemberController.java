...
public class MemberController {
	...
	public String memberTest() {
		...
		// 아래의 view name은 src/main/resources.templates/member/member-test.html을
		// 찾아서 파싱한다
		return "member/member-test";
	}
	@GetMapping("/find-by-id")// url pattern
	// client 에서 보낸 텍스트 입력폼의 name 과 매개변수명이
	// 일치하면 request.getParameter("memberId"); 처리를 해준다 (handler adapter)
	...
	public String findMemberById(@RequestParam String memberId,Model model) {
		...
		// 아래의 view name은 src/main/resources.templates/member/findbyid-result.html로
		// Thymeleaf ViewResolver가 찾아서 파싱한다
	...
	public String findMemberById2(@RequestParam String memId,Model model) {
		if(memId.equals("java")) {
			...
			return "member/findbyid-ok.html";
		}else
			return "member/findbyid-fail.html";
	}
	
	@GetMapping("/param-test")
	public String findMemberAge(@RequestParam String nick,int age, Model model) {
		String type = null;
		if(age<=18) {
			type="미성년";
		}else
	...
	public String register(Member member) { 
		// form 에서 전송한 회원 정보가 객체로 생성되어 매개변수로 전달된다: HandlerAdapter
		System.out.println("회원 가입 정보 등록:"+member);
		//return "member/register-result"; forward 방식: request 유지, 재동작 O
		return "redirect:/member/register-result"; // redirect방식: request 유지X, 재동작 X
	} // 위 코드는 클라이언트에게 localhost:8080/member/register-result 에 가서 응답 받아라
