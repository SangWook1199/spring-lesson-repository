...
public class MyTestController {
	@GetMapping("/hello") // client 가 get 방식으로 요청시 찾는 url pattern 지정
	public String hello(Model model) { // Model: 클라이언트에게 응답할 정보를 공유하기 위한 객체
		...
		// forward방식: ViewResolver에 의해 src/main/resources/template/result1.html
		// 로 제어가 이동되어 타임리프 템플릿 엔진에 의해 파싱되어 클라이언트에게 html 응답한다
		return "result1";
	}

	@GetMapping("/paramTest") // HandlerMapping url 등록해서 찾아줌
	public String paramTest(String customerId, int customerAge, Model model) { // Handler Adapter가 호출
	...
	/**
	 * client에서 post방식으로 요청하여 @PostMapping 으로 처리 client의 form에서 동일한 name (menu)으로 여러
	 * 개의 value를 전송한다 이 경우 Servlet/JSP에서는 request.getParameterValues(): String[]으로
	 * 요청을 처리 이 것을 HandlerAdapter가 요청 데이터에 대한 배열 객체를 생성해서 우리의 컨트롤러 메서드에 전달해준다
	 * 
	 */

	@PostMapping("/paramTest2")
	public String paramTest2(String[] menu, Model model) {

		// redirect 방식은 클라이언트에게 url을 지정해서 응답받게 함
		return "redirect:/result3-view";
		// spring 환경에서 redirectsms frontcontroller 인 dispatcherservlet
		// 으로 가게 되어 있고 적절한 컨트롤러가 있을 때 응답할 수 있다
	}
...
//		return "redirect:/result4-view";
//	}
//	@GetMapping("/result4-view")
//	public String paramTest3result() {
//		return "result4";
//	}
