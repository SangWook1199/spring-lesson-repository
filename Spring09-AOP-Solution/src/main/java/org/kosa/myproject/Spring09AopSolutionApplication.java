...
/**
 * 요구사항 -> AOP를 적용하여 공통 기능을 처리 또는 횡단 관심사를 처리 spring08-AOP-basic 에서 AOP를 사용하지 않은
 * 예와 비교해본다
 * 
 * 요구사항 -> AOP로 처리할 공통 또는 횡단 관심사항(Cross Cutting Concern) 현재 서비스중인 시스템의 검색
 * 계열(find 계열) 기능 메서드들을 대상으로 특정 업무를 메서드 코어 작업 실행 직전에 수행하도록 작업을 추가해야 한다 대상 서비스
 * 클래스들과 메서드들이 다수가 존재하는 것으로 전제하고 공통된 업무 즉 추가 요구 사항을 적용해본다
 * 
 * 검색 계열 메서드 실행전에 ***cross cutting concern***MemberService****findMemberById***
 * 공통 로그 출력
 * 
 * AOP 적용 단계 
 ...
 *    여러 곳을 수정 작업 필요 없고 오직 AOP 모듈만 업데이트하면 된다
 *    높은 응집도 high cohesion : Core는 core에만 집중, cross cutting 은 별도로 전담
...
public class Spring09AopSolutionApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		// 아래는 SpringContainer가 주입해준 Proxy(CGLIB) 대리인 객체
		// AOP가 적용 (Proxy 대리인이 AOP 기능 실행하고 Core 를 실행)
