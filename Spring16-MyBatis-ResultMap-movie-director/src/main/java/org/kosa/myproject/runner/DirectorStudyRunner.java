...
public class DirectorStudyRunner implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		/**
		 * Table Director 의 리스트 정보를 반환받아 출력 DirectorMapper interface 와 DirectorMapper
		 * xml을 정의하여 아래 메서드가 정상적으로 수행하여 출력되도록 구현
		 */
...
//			System.out.println(director);
		...
		for (Map<String, Object> map : list)
			// ROUND(AVG(m.attendance),0) AS averageAttendance 와 AS 로 별칭을 준 이름이 map 의 key로 반환된다
			System.out.println(map.get("directorId") + " " + map.get("directorName") + " " + map.get("totalAttendance")
					+ " " + map.get("averageAttendance"));
	}
