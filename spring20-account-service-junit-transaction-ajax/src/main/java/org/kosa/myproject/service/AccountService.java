...
/**
 * 계좌 관련 비즈니스 로직 처리 서비스 - 계좌 등록 및 조회 기능 제공 - 고객 존재 여부 검증 - 비즈니스 규칙 적용
 */
@Service
@Transactional
public class AccountService {
	...
	/**
	 * 신규 계좌 등록 - 고객 존재 여부 확인 - 계좌 정보 검증 후 등록
	 * 
	...
	/**
	 * 계좌 입력값 검증 private 메서드 - 필수값 체크 및 비즈니스 규칙 검증
	 */
	private void validateAccountInput(Account account) {
		...
		/*
		 * BigDecimal 클래스의 compareTo(BigDecimal val) 메서드는 두 BigDecimal 객체의 수학적인 값을 비교하는
		 * 데 사용 스케일(소수점 이하 자릿수)과 상관없이 오직 값 자체만을 비교 compareTo 메서드는 정수 값을 반환 0: 두
		 * BigDecimal 객체의 값이 같을 때. 1: BigDecimal 객체의 값이 인자로 전달된 BigDecimal 객체보다 클 때. -1:
		 * BigDecimal 객체의 값이 인자로 전달된 BigDecimal 객체보다 작을 때.
		 */
	...
	/**
	 * 계좌 이체 기능 - 출금 계좌에서 입금 계좌로 금액 이체 - 트랜잭션으로 전체 과정을 하나의 단위로 처리 - 실패 시 자동 롤백으로 데이터
	 * 일관성 보장
	 * 
	...
	public void transferMoney(Long fromAccountNumber, Long toAccountNumber, BigDecimal transferAmount) {
		...
		////////////////////////////////////////////////////////////////////////////////
		////////////////////////////////////////////////////////////////////////////////
		//toAccountNumber = 999L; // 주석처리해서 정상 계좌 이체가 되는지 확인
	...
	/**
	 * 이체 입력값 검증 private 메서드 - 계좌번호와 이체금액의 유효성 검사
	 */
	...
	/**
	 * 고객별 계좌 목록 조회 고객의 모든 계좌를 고객 정보와 함께 조회
	 * 
	 * @param customerId
