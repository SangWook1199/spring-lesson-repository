package org.kosa.myproject; // @SpringBootApplication 이 명시된 클래스의 패키지가
...
// 스프링 부트 설정 애너테이션: 설정클래스임을 알림, 컴포넌트 스캔, 자동 설정 지원
...
//	@Autowired // DI 의존성 주입 애너테이션
	...
	public Spring06SpringBootCommandLineRunnerApplication(FoodService foodService) {
		...
		System.out.println(foodService + " 객체: 의존성 주입 DI");
	}

	public static void main(String[] args) {
		System.out.println("Spring Boot App 시작");
		...
		System.out.println("Spring Boot App 종료");
	}

	// CommandLineRunner 는 Spring 의 컨테이너가 실행 준비를 완료한 직후에 실행된다
	// 설정 로딩 및 Bean 생성과 DI 와 같은 작업이 이미 완료한 후에 실행됨
	@Override
